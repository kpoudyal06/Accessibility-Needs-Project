...
server <- function(input, output, session) {
  ...
  # ============================================================================
  # FILE UPLOAD & DATABASE INSERTION (REMOTE)
  # ============================================================================
  observeEvent(input$submit, {
    ...
    # 4. Insert Job into Remote Database
    job_output <- execute_remote_db_command("db_insert_job.sh", 
                                           c(new_job_id, submission_id))
    
    if (!grepl("JOB_INSERT_SUCCESS", paste(job_output, collapse = "\n"))) {
      showNotification("Database Error: Could not insert job", type = "error")
      return()
    }
    
    # 5. Upload Files to Cluster
    wsl_frontend_dir <- file.path("/mnt/c", "Users", win_user, "Downloads", 
                                  "Accessibility-Needs-Project", "frontend")
    ...
    # 6. Save Job ID and show success
    job_id_val(new_job_id)
    submitted(TRUE)
  })
  
  # ============================================================================
  # JOB STATUS QUERY (REMOTE)
  # ============================================================================
  output$job_status <- renderText({
    ...
    job_output <- execute_remote_db_command("db_query_job.sh", c(job_string))
    
    if (any(grepl("JOB_NOT_FOUND", job_output))) {
      return(paste("Error. Job ID", jobId, "does not exist in the system."))
    }
    
    status <- parse_db_result(job_output, "JOB_STATUS")
    if (!is.null(status)) {
      return(paste("Job", jobId, "found. Your file is:", status))
    } else {
      return("Error querying job status.")
    }
  })
